 ...
 * @since 2.0
 */
class Pagination extends ObjectHelper
{
    const REL_SELF = 'self';
    ...
    public function getLinks($absolute = false)
    {
        ...
        $pageCount = $this->getPageCount();
        $links = [
            self::REL_SELF => $this->createUrl($currentPage, null, $absolute),
        ];
        if ($currentPage > 0) {
    ...
    {
        return isset($this->params[$name]) && is_scalar($this->params[$name]) ? $this->params[$name] : $defaultValue;
    }
}
