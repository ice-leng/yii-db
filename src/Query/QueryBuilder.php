...
use Lengbin\YiiDb\ColumnSchemaBuilder;
use Lengbin\YiiDb\Conditions\ConditionInterface;
use Lengbin\YiiDb\Conditions\HashCondition;
use Lengbin\YiiDb\Connection;
use Lengbin\YiiDb\Constraint;
use Lengbin\YiiDb\ConstraintFinderInterface;
...
use Lengbin\YiiDb\ExpressionBuilder;
use Lengbin\YiiDb\ExpressionBuilderInterface;
use Lengbin\YiiDb\ExpressionInterface;
use Lengbin\YiiDb\PdoValue;
use Lengbin\YiiDb\PdoValueBuilder;
use Lengbin\YiiDb\QueryExpressionBuilder;
use Lengbin\YiiDb\Schema;
...
class QueryBuilder
{
    ...
    {
        return [
            'NOT'         => 'Lengbin\YiiDb\Conditions\NotCondition',
            'AND'         => 'Lengbin\YiiDb\Conditions\AndCondition',
            'OR'          => 'Lengbin\YiiDb\Conditions\OrCondition',
            'BETWEEN'     => 'Lengbin\YiiDb\Conditions\BetweenCondition',
            'NOT BETWEEN' => 'Lengbin\YiiDb\Conditions\BetweenCondition',
            'IN'          => 'Lengbin\YiiDb\Conditions\InCondition',
            'NOT IN'      => 'Lengbin\YiiDb\Conditions\InCondition',
            'LIKE'        => 'Lengbin\YiiDb\Conditions\LikeCondition',
            'NOT LIKE'    => 'Lengbin\YiiDb\Conditions\LikeCondition',
            'OR LIKE'     => 'Lengbin\YiiDb\Conditions\LikeCondition',
            'OR NOT LIKE' => 'Lengbin\YiiDb\Conditions\LikeCondition',
            'EXISTS'      => 'Lengbin\YiiDb\Conditions\ExistsCondition',
            'NOT EXISTS'  => 'Lengbin\YiiDb\Conditions\ExistsCondition',
        ];
    }
    ...
    {
        return [
            BaseQuery::class                               => QueryExpressionBuilder::class,
            PdoValue::class                                => PdoValueBuilder::class,
            Expression::class                              => ExpressionBuilder::class,
            'Lengbin\YiiDb\Conditions\ConjunctionCondition'    => 'Lengbin\YiiDb\Conditions\ConjunctionConditionBuilder',
            'Lengbin\YiiDb\Conditions\NotCondition'            => 'Lengbin\YiiDb\Conditions\NotConditionBuilder',
            'Lengbin\YiiDb\Conditions\AndCondition'            => 'Lengbin\YiiDb\Conditions\ConjunctionConditionBuilder',
            'Lengbin\YiiDb\Conditions\OrCondition'             => 'Lengbin\YiiDb\Conditions\ConjunctionConditionBuilder',
            'Lengbin\YiiDb\Conditions\BetweenCondition'        => 'Lengbin\YiiDb\Conditions\BetweenConditionBuilder',
            'Lengbin\YiiDb\Conditions\InCondition'             => 'Lengbin\YiiDb\Conditions\InConditionBuilder',
            'Lengbin\YiiDb\Conditions\LikeCondition'           => 'Lengbin\YiiDb\Conditions\LikeConditionBuilder',
            'Lengbin\YiiDb\Conditions\ExistsCondition'         => 'Lengbin\YiiDb\Conditions\ExistsConditionBuilder',
            'Lengbin\YiiDb\Conditions\SimpleCondition'         => 'Lengbin\YiiDb\Conditions\SimpleConditionBuilder',
            'Lengbin\YiiDb\Conditions\HashCondition'           => 'Lengbin\YiiDb\Conditions\HashConditionBuilder',
            'Lengbin\YiiDb\Conditions\BetweenColumnsCondition' => 'Lengbin\YiiDb\Conditions\BetweenColumnsConditionBuilder',
        ];
    }
     ...
     */
    public function build($query = null, $params = [])
    {
        if ($query === null) {
     ...
     * @param array|BaseQuery $columns the column data (name => value) to be inserted into the table or instance
    ...
    public function insert($table, $columns, &$params)
    {
        ...
        return 'INSERT INTO ' . $this->db->quoteTableName($table) . (!empty($names) ? ' (' . implode(', ',
                    $names) . ')' : '') . (!empty($placeholders) ? ' VALUES (' . implode(', ', $placeholders) . ')' : $values);
    }
    ...
    protected function prepareInsertValues($table, $columns, $params = [])
    {
        ...
        if ($columns instanceof BaseQuery) {
            ...
            foreach ($columns as $name => $value) {
                ...
                } elseif ($value instanceof BaseQuery) {
     ...
     *
     * @param BaseQuery $columns Object, which represents select query.
     ...
     * @param array|BaseQuery $insertColumns the column data (name => value) to be inserted into the table or instance
    ...
    {
        throw new NotSupportedException($this->db->getQuery()->getDriverName() . ' does not support upsert statements.');
    }

    /**
     * @param string          $table
     * @param array|BaseQuery $insertColumns
    ...
    {
        if ($insertColumns instanceof BaseQuery) {
    ...
    public function addPrimaryKey($name, $table, $columns)
    {
        ...
        return 'ALTER TABLE ' . $this->db->quoteTableName($table) . ' ADD CONSTRAINT ' . $this->db->quoteColumnName($name) . ' PRIMARY KEY (' . implode(', ',
                $columns) . ')';
    }
    ...
    public function addUnique($name, $table, $columns)
    {
        ...
        return 'ALTER TABLE ' . $this->db->quoteTableName($table) . ' ADD CONSTRAINT ' . $this->db->quoteColumnName($name) . ' UNIQUE (' . implode(', ',
                $columns) . ')';
    }
    ...
    {
        throw new NotSupportedException($this->db->getQuery()->getDriverName() . ' does not support adding default value constraints.');
    }
    ...
    {
        throw new NotSupportedException($this->db->getQuery()->getDriverName() . ' does not support dropping default value constraints.');
    }
    ...
    {
        throw new NotSupportedException($this->db->getQuery()->getDriverName() . ' does not support resetting sequence.');
    }
    ...
    {
        throw new NotSupportedException($this->db->getQuery()->getDriverName() . ' does not support enabling/disabling integrity check.');
    }
     ...
     * @param string|BaseQuery $subQuery the select statement which defines the view.
    ...
    public function buildSelect($columns, &$params, $distinct = false, $selectOption = null)
    {
        ...
        foreach ($columns as $i => $column) {
            ...
            } elseif ($column instanceof BaseQuery) {
