...
class Connection extends Component implements ConnectionInterface
{
    ...
    public $commandMap = [
        ...
        'sqlsrv'  => 'Lengbin\YiiDb\Command', // newer MSSQL driver on MS Windows hosts
     ...
     */
    public function __construct(array $dsn = null, LoggerInterface $logger = null)
    {
        if (is_array($dsn)) {
     ...
     *                                                           Use 0 to indicate that the cached data will never expire.
     * @param null     $dependency                               the cache dependency associated with the cached query results.
     *
     ...
     * @param null $dependency the preferred caching dependency. If null, it will be ignored.
     *
        ...
        if ($pdoClass === null) {
            ...
            if (isset($driver)) {
                if ($driver === 'mssql' || $driver === 'dblib') {
                    $pdoClass = 'Lengbin\YiiDb\Mssql\PDO';
                } elseif ($driver === 'sqlsrv') {
                    $pdoClass = 'Lengbin\YiiDb\Mssql\SqlsrvPDO';
                }
            }
        }
        $dsn = $this->dsn;
    ...
    public function createCommand($sql = null, $params = [])
    {
        ...
        $config = ['class' => 'Lengbin\YiiDb\Command'];
    ...
    {
        $this->configure($this->getQueryBuilder(), $config);
    }
    ...
    protected function openFromPoolSequentially(array $pool, array $sharedConfig)
    {
        ...
        foreach ($pool as $config) {
            ...
            $key = $this->buildKey([__METHOD__, $config['dsn']]);
